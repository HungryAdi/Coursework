...
: hash(thash != nullptr ? thash : chash), load_threshold(to_copy.load_threshold), bins(to_copy.bins) {
  if (hash == nullptr)
...
: hash(thash != nullptr ? thash : chash), load_threshold(the_load_threshold), bins(std::max(1,int(the_load_threshold*il.size()))) {
  if (hash == nullptr)
...
: hash(thash != nullptr ? thash : chash), load_threshold(the_load_threshold), bins(std::max(1,int(the_load_threshold*i.size()))) {
  if (hash == nullptr)
