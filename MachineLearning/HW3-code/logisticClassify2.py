...
## METHODS ################################################################
    ...
    def plotBoundary(self,X,Y):
        ...
        x2b = (self.theta[0] + self.theta[1]*x1b)/(-1*self.theta[2]);      #find x2 values as a function of x1's values
    ...
    def train(self, X, Y, initStep=1.0, stopTol=1e-4, stopEpochs=5000, plot=None):
        ...
        while not done:
            ...
            for i in np.random.permutation(M):
                ...
                gradi = -YY[i]*(1-self.sig(ri))*XX[i] + (1-YY[i])*XX[i]*(self.sig(ri)) + alpha*np.sum(self.theta**2);     # TODO: compute gradient of NLL loss
